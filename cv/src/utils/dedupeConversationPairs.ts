 ...
 */
export function dedupeConversationPairs(messages: ChatMessage[]): ChatMessage[] {
  ...
  const userAssistantPairs: Array<{ userContent: string; userIndex: number; assistantIndex: number }> = [];
  ...
  const lastOccurrence = new Map<string, { userContent: string; userIndex: number; assistantIndex: number }>();
