...
export function setupFitModal() {
	...
	const overlay = fitModalEl.querySelector(
		".chat-modal-overlay, .fit-modal-overlay",
	);
...
function renderFitResults(
	...
	const moderateIcon =
		'<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="9"/><path stroke-linecap="round" stroke-linejoin="round" d="M12 8v4"/></svg>';
	const warningIcon =
	...
	if (jobPostingJudgment && jobPostingJudgment.isJobPosting === false) {
		const messageText =
			result.jobPostingMessage ||
			"This content doesn't appear to be a job posting.";
		const reasonText = jobPostingJudgment.reason
			? `<p class="fit-job-missing-reason">${escapeHtml(jobPostingJudgment.reason)}</p>`
			: "";
		const confidenceText = jobPostingJudgment.confidence
			? `<p class="fit-job-missing-confidence">Confidence: ${escapeHtml(jobPostingJudgment.confidence)}</p>`
			: "";
		const html = `
	...
	if (isStrong) {
		...
		verdictSubtext =
			"This role has several relevant matches; here are the strengths and areas to watch:";
	} else {
	...
	const jobSnippet =
		jobInput.length > 200 ? `${jobInput.substring(0, 200)}...` : jobInput;
  ...
  `;
	...
	html +=
		'<button type="button" class="fit-new-assessment" onclick="resetFitModal()">Analyze Another Job</button>';
