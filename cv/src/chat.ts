...
function getConversationHistory(): ChatMessage[] {
	try {
		...
		console.warn("Error reading conversation history:", _err);
		return [];
	}
}
...
function saveConversationHistory(messages: ChatMessage[]) {
	try {
		...
		console.warn("Error saving conversation history:", _err);
	}
}
...
export async function loadChatConfig() {
	...
	try {
		...
		console.error("Error loading chat-config.json:", _err);
		chatConfig = createFallbackConfig();
	}
	return chatConfig;
}
...
function createFallbackConfig(): ChatConfig {
	return {
		default: {
			...
			questions: [
				"What are Kris's key skills?",
				"Tell me about Kris's experience",
			],
		},
		topics: {},
	};
...
function getTopicConfig(
	...
	if (section === "skills" && skillId) {
		const skillsTopic = topics.skills as
...
function renderMarkdownContent(element: HTMLElement, markdown: string) {
	...
	try {
		...
		console.warn("renderMarkdownContent error:", _err);
...
function createSlowRenderer(
	...
	try {
		...
		const originalSetAttr = r.set_attr;
		if (typeof originalSetAttr === "function") {
			r.set_attr = function set_attr_wrapper(
				data?: { nodes?: Element[]; index?: number },
				attr?: string | number,
				value?: string,
			) {
	...
	const markListItemHasStreamingContent = (
	...
	const add_text = (
		...
		const target =
			data && Array.isArray(data.nodes) && typeof data.index === "number"
	...
	return {
		...
		add_text,
		waitForIdle,
		// Keep the underlying parser data intact (cast to any to accept differing renderer shapes)
		data: base.data,
	};
}
...
export async function openChatWithMoreInfo(
	section: string | null,
	index: number | null,
	skillId: string | null,
) {
	...
	_currentTopic = { section, index, skillId };
...
export function setupChatScrollIndicator() {
	...
	if (!messages || !chatModalContent || scrollIndicatorEl) {
		return;
	}
	...
	chatModalContent.appendChild(scrollIndicatorEl);
	messages.addEventListener("scroll", updateScrollIndicatorVisibility, {
		passive: true,
	});
	updateScrollIndicatorVisibility();
}
...
async function handleRateLimit(_messagesContainer: Element) {
	...
	try {
		...
		try {
			...
			try {
				setAnchorsOpenNewTab(typedEl as Element);
			} catch (_err) {
				/* ignore */
			}
		} catch (_err) {
			console.warn(
				"Failed to render rate-limit markdown, keeping typed text:",
				_err,
			);
		}
		...
		console.warn("Failed handling rate-limit response (typed fallback):", _err);
		hideTypingIndicator();
		scrollChatToBottom();
	}
}

async function streamResponseToMessage(
	response: Response,
	...
	const ensureBotMessage = () => {
		...
		if (!markdownSetup && botMessageEl) {
			markdownSetup = true;
			try {
				renderer = createSlowRenderer(botMessageEl, {
					delayMs: 20,
					chunkSize: 3,
				});
				if (renderer) parser = smd.parser(renderer);
			} catch (_err) {
				console.warn(
					"streaming-markdown init failed, falling back to plain text streaming",
					_err,
				);
				renderer = null;
				parser = null;
		...
		if (result.value) {
			...
			if (parser && typeof smd.parser_write === "function") {
				try {
					...
					console.warn("parser_write error, appending raw text", _err);
					if (messageEl) {
	...
	if (parser && typeof smd.parser_end === "function") {
		try {
			...
			console.warn("parser_end error", _err);
		}
	}

	const wf = (renderer as IRenderer | null)?.waitForIdle;
	if (wf && typeof wf === "function") {
		await wf();
	}
	...
	if (botMessageEl) {
		fallbackText =
			(botMessageEl as HTMLElement).textContent ||
			(botMessageEl as HTMLElement).innerText ||
			"";
	}
	const assistantTextCandidate = assistantChunks.join("");
	const assistantText = assistantTextCandidate.length
		? assistantTextCandidate
		: fallbackText;
	if (botMessageEl) (botMessageEl as HTMLElement).classList.remove("streaming");
...
export async function sendMessage() {
	...
	if (!message || !input || !messagesContainer) {
		return;
	}
	...
	const existingQuestions = messagesContainer.querySelector(
	...
	try {
		...
		const assistantText = await streamResponseToMessage(
			response,
			messagesContainer,
		);
...
/**
 * Ensure the chat UI shows a short LLM disclaimer on top of the messages area.
 */
function ensureChatDisclaimer() {
	const selectors = [
		"#chat-messages",
		".chat-messages",
		"#chat-modal .messages",
	...
	if (!container) {
		...
		if (modal) {
			container =
				(modal.querySelector(".messages") as HTMLElement | null) || modal;
		}
	}
	if (!container) {
		return;
	}
	if (container.querySelector("[data-llm-disclaimer]")) {
		return;
	}

	const d = document.createElement("div");
	d.setAttribute("data-llm-disclaimer", "1");
	d.className = "chat-disclaimer";
	d.setAttribute("role", "note");
	d.setAttribute("aria-live", "polite");
	d.style.fontSize = "0.9em";
	d.style.padding = "8px 12px";
	d.style.borderBottom = "1px solid rgba(0,0,0,0.06)";
	d.style.color = "var(--muted-text, #444)";
	d.textContent =
		"Note: This chat is generated by an AI (LLM) and may make mistakes or hallucinate. While grounded in 20,000 words of background data, it may occasionally produce incorrect or misleading information. It runs on Cloudflare's web workers which sometimes can take a few seconds to start up.";
