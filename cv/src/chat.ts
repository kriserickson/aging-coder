...
function ensureChatDisclaimer() {
  ...
  if (!container) {
    const modal = document.getElementById('chat-modal');
    if (modal) {
      container = (modal.querySelector('.messages') as HTMLElement | null) || modal;
    }
  }
  if (!container) {
    return;
  ...
  d.textContent = 'Note: This chat is generated by an AI (LLM) and may make mistakes or hallucinate. While grounded in 20,000 words of background data, it may occasionally produce incorrect or misleading information. It runs on Cloudflare\'s web workers which sometimes can take a few seconds to start up.';
